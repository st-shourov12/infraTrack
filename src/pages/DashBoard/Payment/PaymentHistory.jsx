...
const PaymentHistory = () => {
    ...
    const filteredPayments = useMemo(() => {
        ...
        if (amountFilter !== 'all') {
            filtered = filtered.filter(payment => {
                switch (amountFilter) {
                    case 'premium': return payment.amount === 1000;
                    case 'boost': return payment.amount === 100;
                    case 'high': return payment.amount > 500;
                    case 'low': return payment.amount <= 500;
                    default: return true;
                }
            });
        }
    ...
    const getPaymentType = (amount) => {
        if (amount === 1000) return { label: 'Premium', color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300' };
        if (amount === 100) return { label: 'Boost', color: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300' };
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 md:p-8">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">View and manage all your payment transactions</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-gray-600 dark:text-gray-400 text-sm mb-1">Premium Plans</p>
                                <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.premium}</p>
                            </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-gray-600 dark:text-gray-400 text-sm mb-1">Issue Boosts</p>
                                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{stats.boost}</p>
                            </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
                    <div className="flex flex-col lg:flex-row gap-4">
                        ...
                        <div className="flex-1">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    placeholder="Search by transaction ID, tracking ID, or name..."
                                    className="w-full pl-12 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                />
                            </div>
                        </div>
                        ...
                        <button
                            onClick={() => setShowFilters(!showFilters)}
                            className="flex items-center justify-center gap-2 px-6 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-semibold rounded-lg transition-colors duration-300"
                        >
                            <FaFilter />
                            {showFilters ? 'Hide Filters' : 'Show Filters'}
                        </button>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                                <tr>
                                    {['#', 'User', 'Type', 'Amount', 'Transaction ID', 'Tracking ID', 'Date', 'Actions'].map(h => (
                                        <th key={h} className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                            {h}
                                        </th>
                                    ))}
                                </tr>
                                    ...
                                    filteredPayments.map((payment, index) => {
                                        ...
                                        return (
                                            <tr key={payment._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150">
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex flex-col">
                                                        <span className="text-sm font-medium text-gray-900 dark:text-white">{payment.userName}</span>
                                                        <span className="text-xs text-gray-500 dark:text-gray-400">{payment.userEmail}</span>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${paymentType.color}`}>
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className="text-xs font-mono bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded text-blue-700 dark:text-blue-300">
                                                        {payment.trackingId}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <a
                                                        href={`https://infratrack-server.vercel.app/invoice/${payment?._id}`}
                                                        target="_blank"
                                                        className="flex items-center gap-2 px-4 py-2 bg-blue-600 dark:bg-blue-700 hover:bg-blue-700 dark:hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-colors duration-300"
                                                    >
                                                        <FaDownload />
                                                        Invoice
                                                    </a>
